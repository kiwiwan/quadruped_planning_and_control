...
def getAllJointIndicesInGeneralizedPositions(plant):
    for joint_limit in SRBD.JOINT_LIMITS.items():
...
def getJointLimitsSortedByActuator(plant):
    return sorted(SRBD.JOINT_LIMITS.items(), key=lambda entry : getActuatorIndex(plant, entry[0]))
...
def getJointLimitsSortedByPosition(plant):
    return sorted(SRBD.JOINT_LIMITS.items(),
